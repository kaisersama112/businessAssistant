AG（Retrieval Augmented Generation）技术，通过检索与用户输入相关的信息片段，并结合***外部知识库***来生成更准确、更丰富的回答。解决 LLMs 在处理知识密集型任务时可能遇到的挑战, 如幻觉、知识过时和缺乏透明、可追溯的推理过程等。提供更准确的回答、降低推理成本、实现外部记忆。
...
由于茴香豆是一款比较新的应用， `InternLM2-Chat-7B` 训练数据库中并没有收录到它的相关信息。左图中关于 huixiangdou 的 3 轮问答均未给出准确的答案。右图未对 `InternLM2-Chat-7B` 进行任何增训的情况下，通过 RAG 技术实现的新增知识问答。
...
茴香豆工具在 `Intern Studio` 开发机的安装工作结束。如果部署在自己的服务器上，参考上节课模型下载内容或本节 [3.4 配置文件解析](https://github.com/InternLM/Tutorial/blob/camp2/huixiangdou/readme.md#34-配置文件解析) 部分内容下载模型文件。
...
配置文件具体含义和更多细节参考 [3.4 配置文件解析](https://github.com/InternLM/Tutorial/blob/camp2/huixiangdou/readme.md#34-配置文件解析)。
...
[![img](https://github.com/InternLM/Tutorial/raw/camp2/huixiangdou/imgs/huixiangdou.png)](https://github.com/InternLM/Tutorial/blob/camp2/huixiangdou/imgs/huixiangdou.png)

[![img](https://github.com/InternLM/Tutorial/raw/camp2/huixiangdou/imgs/install.png)](https://github.com/InternLM/Tutorial/blob/camp2/huixiangdou/imgs/install.png)

`InternLM2-Chat-7B` 的关于 `huixiangdou` 问题的原始输出：

[![img](https://github.com/InternLM/Tutorial/raw/camp2/huixiangdou/imgs/internlm27b.png)](https://github.com/InternLM/Tutorial/blob/camp2/huixiangdou/imgs/internlm27b.png)
...
[![img](https://github.com/InternLM/Tutorial/raw/camp2/huixiangdou/imgs/overall.png)](https://github.com/InternLM/Tutorial/blob/camp2/huixiangdou/imgs/overall.png)
...
2. 在本地浏览器中输入 [127.0.0.1:7860](http://127.0.0.1:7860/) 进入 **Gradio** 对话 Demo 界面，开始对话。

如果需要更换检索的知识领域，只需要用新的语料知识重复步骤 [2.2 创建知识库](https://github.com/InternLM/Tutorial/blob/camp2/huixiangdou/readme.md#22-创建知识库) 提取特征到新的向量数据库，更改 `huixiangdou/config.ini` 文件中 `work_dir = "新向量数据库路径"`；
